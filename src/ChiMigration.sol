 ...
 * And we also derive the associated EVM address (via Keccak / truncation)
...
contract ChiMigration is MerkleClaim, Ownable, EIP712
{
      ...
      public
  {
    ...
    /* Check that the public key provided matches the Xaya pubkey hash.  */
    if (ripemd160 (abi.encodePacked (sha256 (pubkey))) != utxo.pubkeyhash)
      revert InvalidClaimSignature (utxo.txid, utxo.vout);

    /* Derive the EVM address corresponding to this same public key.  */
    bytes32 keccakHash = keccak256 (pubkey);
