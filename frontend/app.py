...
def is_output_claimed(txid, vout):
    """Check if an output has been claimed already."""
    if migration_contract is None:
        raise RuntimeError("Migration contract not initialized")
    
    # Calculate the UTXO identifier
    identifier = utxo_identifier(txid, vout)
    
    # Query the contract
    claimed_address = migration_contract.functions.claimedOutputs(identifier).call()
    
    # Address 0x0 means not claimed
    return claimed_address != '0x0000000000000000000000000000000000000000'
    ...
    if request.method == 'POST':
        ...
        if utxo_set:
            ...
            for idx in output_indices:
                ...
                # Check if this output has been claimed
                output_claimed = is_output_claimed(output['txid'], output['vout'])
                ...
                claim = {
                    ...
                    'claimed': output_claimed,  # Now checking real claim status
